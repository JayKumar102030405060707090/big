...
bot_token = "7614916680:AAEVOATGqMzDOfIpDCJ36_1K_7ONnt2vXis"
...
def check_force_subscribe(user_id):
    try:
        for username in channel_usernames:
            member = app.get_chat_member(username, user_id)
...
def send_force_subscribe(user_id):
    try:
        app.send_message(
            user_id, 
            "Aapko neeche diye gaye sabhi channel join karne honge:",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("🔗 Channel 1", url=force_subscribe_channels[0]),
                    InlineKeyboardButton("🔗 Channel 2", url=force_subscribe_channels[1])
                ],
                [
                    InlineKeyboardButton("🔗 Channel 3", url=force_subscribe_channels[2]),
                    InlineKeyboardButton("✅ Maine Join Kar Liya", callback_data="check_sub")
                ]
            ])
        )
    except Exception as e:
        logger.error(f"Error in send_force_subscribe: {e}")

# Handle "✅ Maine Join Kar Liya"
@app.on_callback_query(filters.regex("check_sub"))
def check_subscription_callback(client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    try:
        if check_force_subscribe(user_id):
            send_main_menu(user_id, callback_query.from_user.first_name)
        else:
            callback_query.answer("Abhi tak aapne sabhi channels join nahi kiye.", show_alert=True)
    except Exception as e:
...
def agent_work(client, callback_query):
    ...
    try:
        app.send_message(
            user_id, 
            "Agent banne ke liye @INNOCENT_FUCKER se contact karein.\nDaily earning ₹500 - ₹50,000."
        )
    except Exception as e:
        logger.error(f"Error in agent_work: {e}")
